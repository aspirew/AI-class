    ...
    if(os.path.exists('lfw')):
        for dir in os.listdir('lfw'):
            for file in os.listdir(os.path.join('lfw', dir)):
                ...
                NEW_PATH = os.path.join('lfw2', file)
...
def run_camera_for_collection():
    ...
    while cap.isOpened():
        ...
        frame = frame[300:300+250, 550:550+250, :]
...
def reload_model(filename):
    return tf.keras.models.load_model(filename, custom_objects={'L1Dist': L1Dist, 'BinaryCrossentropy':tf.losses.BinaryCrossentropy})
...
model = reload_model('gender-prediction.h5')

evaluate(model, test_data)
# save_model(model, 'gender-prediction.h5')
# model = reload_model('gender-prediction.h5')
